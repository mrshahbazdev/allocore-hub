  ...
  <AppShell>
    <div class="space-y-6">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-2xl font-bold text-gray-900">{{ $t('kpi.catalog') }}</h1>
          <p class="text-sm text-gray-500">{{ $t('kpi.catalog_desc') }}</p>
        </div>
      </div>
          ...
          >
            <div class="flex items-start justify-between mb-2">
              ...
              <form @submit.prevent="useTemplate(tpl.id)">
                <button
                  type="submit"
                  class="text-xs px-2.5 py-1 bg-primary-50 text-primary-700 rounded-lg hover:bg-primary-100 font-medium flex-shrink-0"
                >
                  {{ $t('kpi.use_template') }}
                </button>
              </form>
            ...
            <div class="flex items-center gap-2 text-[10px] text-gray-400">
              <span v-if="tpl.formula" class="font-mono bg-gray-50 px-1.5 py-0.5 rounded">{{ tpl.formula }}</span>
              <span v-if="tpl.unit" class="bg-gray-50 px-1.5 py-0.5 rounded">{{ tpl.unit }}</span>
            </div>
          </div>
        </div>
      </div>
