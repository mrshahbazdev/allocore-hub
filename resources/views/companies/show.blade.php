...
<div class="company-show-grid">
    <div class="card" style="align-self:start;">
        ...
        @foreach([
            ...
            [__('Currency'), $company->currency ?? 'EUR'],
            [__('Country'), $company->country ?? '—'],
            [__('Created'), $company->created_at->format('d.m.Y')],
        ] as [$k,$v])
    ...
    <div class="card">
        <div class="company-analyses-head">
            <div class="card-title" style="margin-bottom:0;">{{ __('Analyses') }} ({{ $analyses->count() }})</div>
            <a href="{{ route('gmbh.create') }}" class="btn btn-primary btn-sm">+ {{ __('New Analysis') }}</a>
        </div>
        @if($analyses->isEmpty())
            <div style="text-align:center; padding:30px; color:#475569;">{{ __('No analyses for this company yet.') }}</div>
        @else
        <div class="company-analyses-table-wrap">
        <table class="data-table">
            <thead><tr><th>{{ __('Name') }}</th><th>{{ __('Type') }}</th><th>{{ __('Score') }}</th><th>{{ __('Date') }}</th><th></th></tr></thead>
            <tbody>
